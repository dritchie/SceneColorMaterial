...
\remark{Introduction (We have all of these predictive functions; now we need to combine them into one function that can tell us the overall quality of a coloring. Factor graphs to the rescue!)}

\remark{Give the graphical definition of the model first, with the picture. Then give the math for each factor.}

\remark{Finally, give the algebraic definition of the model (``this is the distribution encoded by this model''), with the math for unary, binary, and higher-order terms.}

\remark{Sampling, Weight Learning, and Implementation sections remain largely unchanged.}
