...
The unary and binary coloring scoring functions defined in the previous sections indicate whether an individual pattern region is well-colored or whether two adjacent regions are colored well in concert, but they have no knowledge of the global harmony between all colors in the pattern. To enforce global consistency, we include a color compatibility function based on the model introduced by O'Donovan et al.~\shortcite{ODonovan}. Their model predicts 0-5 numeric aesthetic ratings for five-color `color themes,' which are ordered rows of five colors. 
