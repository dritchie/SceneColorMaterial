...
\begin{equation*}
\factor(\colorVars(\group) | \pattern) = \mathcal{N}(||\colorVars(\group) - \textrm{targetColor}(\group)||, \sigma_\textrm{user})
\end{equation*}
...
\begin{equation*}
\termStats(\colors | \pattern) = \sum_{\group \in \groups}{\ln \mathcal{N}(||\colors(\group) - \textrm{targetColor}(\group)||, \sigma_\textrm{user})}
\end{equation*}
