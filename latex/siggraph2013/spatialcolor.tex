...
The model can also be interpreted graphically as a factor graph, where the variables are the group colors in the pattern, $\colorVars$. Each type of term contributes different factors to the graph. For example, the color compatibility term contributes a factor touching at most 5 group color variables that scores the compatibility of those colors. The spatial terms contribute a unary factor for each group color variable and pairwise factors for group color variables that are adjacent in the pattern.

Figure~\ref{fig:FactorGraph} shows an example of a factor graph for a given pattern template. Circles denote the color variables, while squares denote the different factors $\factor$, and edges connect each factor to variables within their scope. 

\begin{figure}[htb]
\centering
%\includegraphics[width=\columnwidth]{}
TODO. 
%Idea: Factor graph for pattern 499194 from sugar (or another simple pattern), and possibly also show the pattern itself to illustrate what the structure means).  
\caption{A factor graph from our model.\remark{S: TODO: Make figure. I can take a shot at a rough version a little later}}
\label{fig:FactorGraph}
\end{figure}

In the rest of this section, we will first detail the individual terms in the model-- the color compatibility term (Section~\ref{sec:colorCompat}) and the spatial terms (Section~\ref{sec:spatialCompat})-- and describe how they generate factors. We discuss how to sample from the model (Section~\ref{sec:sampling}) and how to automatically tune its weights (Section~\ref{sec:weights}). Finally, we provide details on our prototype implementation (Section~\ref{sec:implementation}).
...
In the factor graph representation of our model, this term corresponds to the color compatibility factor that touches the 5 color variables with the largest group size (or fewer if there are less than 5 groups in the pattern). Formally:
\begin{equation*}
\factor(\textrm{top}(\colorVars)) = \exp\left(\frac{\colorCompatWeight \cdot \colorCompatTerm}{t}\right)
\end{equation*}
where $\textsl{top}$ are the color variables of the largest 5 groups in the pattern, and $t$ is the temperature of the model as mentioned before.
...
These spatial terms score color assignments based on the conditional probability a color property $\prop$ (e.g. lightness or colorfulness) given the features of an object $x$ (which is either a group, segment, or segment adjacency). Formally, the term statistics function is defined:
%%
\begin{equation*}
\termStats(\colors | \pattern) = \sum_{x \in \pattern} \termInstStats(x, \colors|\pattern)
\end{equation*}
where
\begin{equation*}
\termInstStats(x, \colors| \pattern) = p(\prop(\colors(x)) | \features(x)) \cdot s_x
\end{equation*}
is the object statistics for that term. Object statistics are used when generating factors from each term.

%%
We compute these probabilities on color properties instead of directly on colors to allow for more generalization over colors that do not occur in the model's training data. $s_x$ is the contribution of $x$ to the overall score, and is based on the impact $x$ has on the appearance of the pattern (i.e. the area of a group or segment or the strength of an adjacency).
...
\end{equation*}
where $\size_\group$ is the size of the group $\group$ and $\features(\group)$ are the features of the $\group$ (See the Appendix for the complete list of features used). We weight the contribution of each group to the term statistics by its relative area as larger regions tend to have more impact on the appearance of a coloring. The inner-sum term is the group object statistic $\groupInstStats(g)$. 
...
Similarly, the segment term for property $\prop$ has the statistics function:
 ...
 \end{equation*}
where $\size_\segment$ is the size of the segment $\segment$ and $\features(\segment)$ are the features of $\segment$. The inner-sum term is the segment object statistic $\segInstStats(s)$.

We generate factors over each color variable by grouping together all term object statistics for the group and segments associated with the variable. Formally:
 \begin{equation*}
...
The sufficient statistics function for each binary property is:
 ...
 \end{equation*}
 where $\adjStrength(\segment,\segprime)$ is the adjacency strength of $(\segment,\segprime)$. Here, we define adjacency strength $\adjStrength(\segment, \segprime)$ as the number of pixels from segments $\segment$ or $\segprime$ that are within a 2-pixel distance from their perimeters. All adjacency strengths are normalized to sum up to 1.  

 %~\remark{D: Define adjacency strength?}
Again, we define the inner-sum term to be the adjacency object statistic $\adjInstStats$.
